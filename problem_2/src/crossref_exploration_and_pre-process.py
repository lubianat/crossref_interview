...
def main():
    ...
    group_member = crossref_df.groupby('DOI_Prefix').agg(
    ...
    fig_top15_rors = px.bar(top15_rors, x='ROR_ID', y='count',
                            title="Top 15 Most Used RORs",
                            labels={'ROR_ID': 'ROR ID', 'count': 'Usage Count'})
    
    fig_ror_usage_bins, fig_ror_usage_bins_unique = generate_figs_for_ror_usage(ror_counts, total_usage_bin)
    
    fig_doi_ror_bins, fig_box_rors_per_doi = generate_figs_for_ror_per_doi(doi_ror_counts)
    ...
    no_affiliation_string = crossref_df['Affiliation_Name'].apply(lambda x: pd.isna(x) or str(x).strip() == "")
    proportion_missing_affiliation_string = no_affiliation_string.sum() / len(crossref_df)
    proportion_containing_affiliation_string = 1 - (no_affiliation_string.sum() / len(crossref_df))
    ...
    crossref_df['normalized_name'] = crossref_df['Affiliation_Name'].apply(helper.normalize_spaces)
    crossref_df['normalized_name'] = crossref_df['normalized_name'].apply(helper.normalize_string)
    crossref_df.to_csv(DATA.joinpath("crossref_ror_ids_with_affiliation_strings.csv"), index=False)
      
    valid_affiliations = crossref_df['Affiliation_Name'].dropna().map(helper.normalize_spaces).replace("", np.nan).dropna()
    ...
    fig_top10_affiliations = px.bar(top10_affiliations, 
                                    x='Affiliation_Name', 
                                    y='count',
                                    title="Top 10 Most Common Affiliation Names",
                                    labels={'Affiliation_Name': 'Affiliation Name', 'count': 'Count'})
    
    fig_affiliation_hist = px.histogram(affiliation_counts, x='count', nbins=30,
    ...
    fig_affil_pie = px.pie(affil_presence_counts, names='Affiliation_Presence', values='count',
                           title="Proportion of Entries with vs. without Affiliation Names")
    
    fig_bubble = px.scatter(group_member, x='total_entries', y='unique_rors', 
                            size='total_entries', color='DOI_Prefix',
    ...
    fig_ror_names_distribution = px.histogram(ror_names, x='num_names', nbins=20,
                                              title="Distribution of Different Affiliation Names per ROR",
                                              labels={'num_names': 'Number of Different Affiliation Names', 'count': 'Frequency'})
    
    top10_ror_diff_names = ror_names.sort_values(by='num_names', ascending=False).head(10)
    top10_table_html = top10_ror_diff_names.to_html(index=False, 
    ...
    affiliation_ror_counts = crossref_df.groupby('normalized_name').agg(
        unique_rors=('ROR_ID', 'nunique'),
        total_occurrences=('ROR_ID', 'count')
    ).reset_index()
    
    ambiguous_affiliations = affiliation_ror_counts[affiliation_ror_counts['unique_rors'] > 1]
    top10_ambiguous_affiliations = ambiguous_affiliations.sort_values(by='unique_rors', ascending=False).head(10)
    
    fig_top10_ambiguous = px.bar(top10_ambiguous_affiliations, x='normalized_name', y='unique_rors',
    ...
    fig_affiliation_scatter = px.scatter(ambiguous_affiliations, 
                                         x='total_occurrences', 
                                         y='unique_rors', 
    ...
    fig_hist_unique_rors = px.histogram(affiliation_ror_counts, x='unique_rors', nbins=30,
                                       title="Distribution of Unique RORs per Affiliation Name",
                                       labels={'unique_rors': 'Number of Unique RORs'})
    
    top10_ambiguous_table_html = top10_ambiguous_affiliations.to_html(index=False, 
    ...
    fig_affiliation_length = px.histogram(crossref_df, x='affiliation_length', nbins=30,
   ...
   # Distribution of Unique Affiliation Names per DOI
    affiliations_per_doi = crossref_df.groupby('DOI')['Affiliation_Name'].apply(lambda x: len(set(map(helper.normalize_spaces, x.dropna()))))
    fig_affiliations_per_doi = px.histogram(affiliations_per_doi, nbins=20,
    ...
    doi_table_html = doi_df[['DOI_Clickable']].to_html(index=False, header=["DOI"], escape=False)
    
    # Crossref Member Details (Top 10 Members)
    group_member_top = group_member.sort_values(by='total_entries', ascending=False).head(5).copy()
    ...
    member_table_html = group_member_top.to_html(index=False, 
    ...
    ror_registry_file = DATA / "v1.59-2025-01-23-ror-data/v1.59-2025-01-23-ror-data_schema_v2.csv"
    ...
    ror_registry_df = ror_registry_df[['id', 'names.types.acronym', 'names.types.alias', 'names.types.label', 'names.types.ror_display', 'relationships', 'external_ids.type.wikidata.all']]
    ...
    html_parts = []
    html_parts.append("<html><head><title>Affiliation Data Analysis</title></head><body>")
    ...
    html_parts.append(pio.to_html(fig_top_members, full_html=False, include_plotlyjs=False))
    
    # Append new ROR usage plots
    html_parts.append("<h2>Most Used RORs</h2>")
    html_parts.append(pio.to_html(fig_top15_rors, full_html=False, include_plotlyjs=False))
    html_parts.append("<h3>Total ROR Occurrences by Usage Bin</h3>")
    html_parts.append(pio.to_html(fig_ror_usage_bins, full_html=False, include_plotlyjs=False))
    html_parts.append("<h3>Unique ROR Count by Usage Bin</h3>")
    html_parts.append(pio.to_html(fig_ror_usage_bins_unique, full_html=False, include_plotlyjs=False))
    
    # Append custom binning for RORs per DOI
    html_parts.append("<h2>RORs per DOI (Custom Bins)</h2>")
    html_parts.append(pio.to_html(fig_doi_ror_bins, full_html=False, include_plotlyjs=False))
    
    # Append Affiliation Name metrics
    html_parts.append("<h2>Affiliation Name Metrics</h2>")
    html_parts.append(pio.to_html(fig_top10_affiliations, full_html=False, include_plotlyjs=False))
    html_parts.append(pio.to_html(fig_affiliation_hist, full_html=False, include_plotlyjs=False))
    
    # Append additional insights
    html_parts.append("<h2>Additional Insights</h2>")
    html_parts.append(pio.to_html(fig_affil_pie, full_html=False, include_plotlyjs=False))
    html_parts.append(pio.to_html(fig_box_rors_per_doi, full_html=False, include_plotlyjs=False))
    ...
    html_parts.append(pio.to_html(fig_ror_names_distribution, full_html=False, include_plotlyjs=False))
    ...
    html_parts.append(pio.to_html(fig_top10_ambiguous, full_html=False, include_plotlyjs=False))
    html_parts.append("<h3>Scatter Plot: Occurrences vs. Unique RORs for Ambiguous Affiliations</h3>")
    html_parts.append(pio.to_html(fig_affiliation_scatter, full_html=False, include_plotlyjs=False))
    html_parts.append("<h3>Distribution of Unique RORs per Affiliation Name</h3>")
    html_parts.append(pio.to_html(fig_hist_unique_rors, full_html=False, include_plotlyjs=False))
    ...
    html_parts.append(pio.to_html(fig_affiliation_length, full_html=False, include_plotlyjs=False))
    ...
    html_parts.append(pio.to_html(fig_affiliations_per_doi, full_html=False, include_plotlyjs=False))
...
def generate_figs_for_ror_per_doi(doi_ror_counts):
    ...
    doi_ror_bins['RORs_per_DOI_bin'] = pd.Categorical(doi_ror_bins['RORs_per_DOI_bin'], 
                                                      categories=bin_order, ordered=True)
    doi_ror_bins = doi_ror_bins.sort_values('RORs_per_DOI_bin')
    
    fig_doi_ror_bins = px.bar(doi_ror_bins, x='RORs_per_DOI_bin', y='count',
                              title="Distribution of RORs per DOI (Custom Bins)",
                              labels={'RORs_per_DOI_bin': 'Number of RORs per DOI', 'count': 'Number of DOIs'})
    
    fig_box_rors_per_doi = px.box(x=doi_ror_counts, points="all",
...
def generate_figs_for_ror_usage(ror_counts, total_usage_bin):
    unique_ror_bin_counts = ror_counts.groupby('usage_bin').size().reset_index(name='unique_ror_count')
    usage_bin_order = ["1", "2-5", "5-10", "10-100", "101-1k", "1k-5k", "5k+"]

    total_usage_bin['usage_bin'] = pd.Categorical(total_usage_bin['usage_bin'], categories=usage_bin_order, ordered=True)
    total_usage_bin = total_usage_bin.sort_values('usage_bin')
    
    unique_ror_bin_counts['usage_bin'] = pd.Categorical(unique_ror_bin_counts['usage_bin'], categories=usage_bin_order, ordered=True)
    unique_ror_bin_counts = unique_ror_bin_counts.sort_values('usage_bin')


    fig_ror_usage_bins = px.bar(total_usage_bin, x='usage_bin', y='count',
                                title="Distribution of Total ROR Occurrences by Usage Bin",
                                labels={'usage_bin': 'Usage Bin', 'count': 'Total Occurrences'})
    
    fig_ror_usage_bins_unique = px.bar(unique_ror_bin_counts, x='usage_bin', y='unique_ror_count',
